...
We implement this in an R package which supports normal, Poisson, DESeq2-based (negative binomial with sample-specific size factors), and empirical (for ordinal data) marginal distributions.
...
Our goal is to be able to generate simulated datasets that look the same as this reference dataset, on average and when $n$ samples were simulated.
...
Indeed, we consider a choice of $\Sigma_{\text{sim}}$ to be successful if datasets of $n$ samples simulated using $\Sigma_{\text{sim}}$ have sample covaraince matrices $\widehat \Sigma_{\text{sim}}$ that are similar to $\widehat \Sigma_{\text{ref}}$.
...
We therefore need to "inject" some level of additional randomness that is independent between genes on top of the correlations captured by $\widehat \Sigma_{\text{ref}}$
...
An additional advantage of this special form is that data may be generated very efficiently, @fig-intro b,c, compared to arbitrary $\Sigma_{\text{sim}}$ which typically require a Cholesky or eigenvalue decomposition.
...
We further demonstrated our methods on metabolomics data, where we use normal marginal distributions and with just 42 features measured, see Supplemental Figure 3.
...
We used a fly whole body RNA-Seq dataset GSE81142, see Supplemental Figure 1.
...
Below, we describe the three methods for selecting the components of the covariance matrix $\Sigma_{\text{sim}} := D^2 + U W^2 U^T$.
...
Here we have described the well-known Gaussian copula approach and argued that a specific form of covariance matrix which is well tailored to omics data simulation.
...
We demonstrated that our methods are highly efficient and easily scale to the large feature counts typical of omics data sets.
...
Moreover, high memory use of the alternatives could be an even larger barrier than run-time.
...
In the second application, CYCLOPS performance in estimating circadian phases depends upon gene-gene dependence, was sensitive to dependence structure of the data.
...
Possibly, this could demonstrate the limitations of methods based on the multivariate normal distribution or of the low-rank approximation used by all three proposed methods. 
