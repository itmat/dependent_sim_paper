...
In omics, choosing this $\Sigma$ presents a challenge since $n < p$.
...
However, using $\widehat Sigma$ is a poor choice for $\Sigma$.
Indeed, we consider a choice of $\Sigma$ to be successful if data sets of $n$ samples simulated using $\Sigma$ have sample covaraince matrices that are similar to $\widehat \Sigma$.
Counter-intuitively, data simulated using $\Sigma := \widehat \Sigma$ have sample covariance matrices that systematically and substantially differ from $\widehat \Sigma$, see @fig-intro.

```{r}
#| label: fig-intro
#| fig-cap: "Eigenvalues of the sample covariance matrix $\\widehat \\Sigma$ when estimated from the reference data set and when estimated from simulated data using $\\Sigma$ equal to $\\widehat \\Sigma$ of the reference data set ('simulated'). Eigenvalues are numbered by size. 100 genes and 12 samples were used for both reference and simulated data sets. Both reference and simulated data follow a multivariate normal distribution."
#| fig-width: 5
#| fig-height: 4
...
This is due to the fact that $\widehat \Sigma$ is low rank, specifically at most rank $n - 1$, which is less than $p$ in typical omics studies.
This limitation means that any data generated from the multivariate normal distribution of that $\Sigma$ will lie on a $n-1$-dimensional hyperplane.
Therefore, newly simulated samples will actually be linear combination of the samples of the reference data set.
Intuitively, this means that the entire probability distribution of possible $p \times n$ dimensional omics data sets have been squished into a $n-1$ dimensional space, generating much higher gene-gene correlations than in the reference data set.
...
We present three methods - called here PCA, spiked Wishart, and corpcor - for making this choice of $\Sigma$, all which take the form
$$ \Sigma = D^2 + PP^T $$
where $D$ is a diagonal matrix capturing the injected independence and $PP^T$ is a low-rank correlation matrix derived from $\widehat \Sigma$.
An additional advantage of this special form is that data may be generated very efficiently, see Methods, compared to arbitrary $\Sigma$ which typically require a Cholesky or eigenvalue decomposition.
...
Below, we describe the three methods for selecting the components of the covariance matrix $\Sigma := D^2 + P P^T$.
...
Moreover, for the form $\Sigma = D^2 + U W U^T$ where $p$ is very large, each column of $U$ is typically very close to orthogonal to any $e_i$, a standard basis vector.
Therefore, when $D = c^2I$ for some constant $c$, we can approximate $\Sigma$ as having $k$ arbitrary eigenvalues from $W$ and $n$ remaining eigenvalues all equal to $c$ corresponding to $D$.
...
2. Compute $w_1, \ldots, w_k$ and $c$ by stochastic gradient descent minimizing $\sum_{i} (E[\lambda'_i] - \lambda_i)^2$ for $\Sigma$ diagonal with entries $w_1, \ldots, w_k, c^2, \ldots, c^2$ [@wishart].
...
Since the sample covariance matrix is rank at most $n$, we again obtain a matrix of the form $\Sigma = D^2 + UWU^T$.
...
Therefore, SPsimSeq induces correlation in the first multivarainte normal draw, but then weakens it in the second independent uniform step.
