...
It is important to note that using $\widehat Sigma$ itself is a poor choice for $\Sigma$.
Indeed, we consider a choice of $\Sigma$ to be successful if data sets simulated using $\Sigma$ have sample covaraince matrices that are similar to $\widehat \Sigma$.
Counter-intuitively, data simulated using $\Sigma := \widehat \Sigma$ have sample covariance matrices that systematically and substantially differ from $\widehat \Sigma$.
...
Intuitively, this means that the entire probability distribution of possible $p \times n$ dimensional omics data sets have been squished into a $n-1$ dimensional space, generating much higher gene-gen correlations than in the reference data set.
...
For example, RNA-seq involves a random sampling of fragments from the true sample of RNA molecules and other sources involve various background noise effects.
