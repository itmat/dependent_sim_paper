...
Moreover, it supports multi-modal data such as for multi-omics.
...
Surprisingly, we found that including dependence in the data set improved the performance of CYCLOPS.
...
We chose as a references dataset, 12 mouse cortex RNA-seq samples from accession GSE151923 [@Wang2022].
...
We then randomly select 5% of the genes to be up or down regulated, with $\log_2$ fold change uniformly distributed between $0.2$ and $2.0$, and simulate 5 "experimental" samples.
This was repeated 20 times for each of four dependence conditions (independent, PCA $k=2$, Wishart $k=11$, and corpcor).

Finally, we ran DESeq2 on each simulated 5 vs 5 experiment and compared the output false discovery rate (FDR) with the true percentages of genes that are differential expressed (@fig-DESeq2 a-b).
We observed that DESeq2 is anti-conservative on both data sets, with similar mean true FDRs for each estimated FDR cutoff.
...
We then simulated differential expression experiments as above and observed a similar result (@fig-DESeq2 c-d) as for the fly whole body data sets.
...
Since by default CYCLOPS performs dimension reduction to the number of dimensions needed to capture 99% of the variance, we hypothesize that data with dependence allows fewer dimensions to be used which improved the performance.
...
The generated data $Z'$ has covariance matrix $\Sigma = D^2 + U W U^T$.
...
These observations demonstrate that there is room for future improvements over independent data in these techniques and possibly demonstrate that the datasets are too far from a multivariate normal distribution even after normalization. 
