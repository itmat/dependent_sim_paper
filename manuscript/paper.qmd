...
Overall, CYCLOPS performance is worse in the corpcor and Wishart data than in the independent data, but surprisingly the pca data shows better performance.
Since by default CYCLOPS performs dimension reduction to the number of dimensions needed to capture 80% of the variance, this performance difference is likely partially due to the number of eigengenes needed by CYCLOPS to meet the variance threshold (@fig-CYCLOPS b).
