...
However, when dropping CYCLOPS to require just 1% variance in each eigengene, we found that its accuracy depends significantly on the dependence structure of the simulated time series data (@fig-CYCLOPS a).
...
In this application, the differences between corpcor and Wishart were such that, while Wishart improved upon the number of eigengenes used compared to corpcor, both undershot the level of correlation in the real data set (see Supplemental Figure 2 d) and the difference between them was too small to affect the actual accuracy of CYCLOPS as measured by the circular correlation.
...
Metabolomics data was obtained from MetaboAnalyst (https://www.metaboanalyst.ca/MetaboAnalyst/upload/RocUploadView.xhtml).
